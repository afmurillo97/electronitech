<?php
	try{
		...
		function totalActivos() {
			...
			$sql=$con->prepare('SELECT COUNT(*) FROM tipoVariable WHERE fechaEliminacion IS NULL');
		...
		function getTipoVariables($inicio, $fin) {
			require 'conexion.php';

			$sql=$con->prepare('SELECT * FROM tipoVariable WHERE fechaEliminacion IS NULL LIMIT :P1,:P2');
		...
		if(isset($_POST['accion'])) {
			switch ($_POST['accion']) {
				case 'ingresar':
					$sql=$con->prepare('INSERT INTO tipoVariable (nombre, descripcion) VALUES (:P1,:P2)');
					...
					break;
				case 'buscador':
					$sql=$con->prepare('SELECT * FROM tipoVariable WHERE nombre LIKE "%":P1"%"');
						...
						foreach ($resultado as $fila) {
							...
							echo '
								<tr>
									...
									<td>
										<button type="button" class="btn btn-warning btn-sm formEditarTipoVariable" data-toggle="modal" data-target="#exampleModal" title="Editar Tipo de Variable" '.$editar.'>
				...
				case 'habilitar':
					if ($_POST['habilitado']==0) {
						$sql=$con->prepare('UPDATE tipoVariable SET fechaEliminacion=NOW() WHERE id=:P1');
					}else{
						$sql=$con->prepare('UPDATE tipoVariable SET fechaEliminacion=NULL WHERE id=:P1');
					}
					...
					break;
				case 'especifico':
					$sql=$con->prepare('SELECT * FROM tipoVariable WHERE id=:P1');
					$resultado=$sql->execute(array('P1'=>$_POST['id']));
					$resultado=$sql->fetchAll();
					$num=$sql->rowCount();
						...
						foreach ($resultado as $fila) {
							echo '
								<div class="modal-header">
									<h5 class="modal-title" id="exampleModalLabel">Editar Tipo de Variable</h5>
								...
								<div class="modal-body">
									<form class="forms-sample">
										...
										<button type="button" class="btn btn-primary mr-2" id="editarTipoVariable" data-dismiss="modal">Guardar</button>
					...
					break;
				case 'editar':
					$sql=$con->prepare('UPDATE tipoVariable SET nombre=:P2, descripcion=:P3 WHERE id=:P1');
