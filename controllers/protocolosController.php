<?php
	try{
		...
		function totalActivos() {
			...
			$sql=$con->prepare('SELECT COUNT(*) FROM protocolos WHERE fechaEliminacion IS NULL');
		...
		function getProtocolos($inicio, $fin) {
			require 'conexion.php';

			$sql=$con->prepare('SELECT * FROM protocolos WHERE fechaEliminacion IS NULL ORDER BY nombre LIMIT :P1,:P2');
		...
		if(isset($_POST['accion'])) {
			switch ($_POST['accion']) {
				case 'ingresar':
					$sql=$con->prepare('INSERT INTO protocolos (nombre, descripcion) VALUES (:P1,:P2)');
					...
					break;
				case 'buscador':
					$sql=$con->prepare('SELECT * FROM protocolos WHERE nombre LIKE "%":P1"%"');
				...
				case 'habilitar':
					if ($_POST['habilitado']==0) {
						$sql=$con->prepare('UPDATE protocolos SET fechaEliminacion=NOW() WHERE id=:P1');
					}else{
						$sql=$con->prepare('UPDATE protocolos SET fechaEliminacion=NULL WHERE id=:P1');
					}
					...
					break;
				case 'especifico':
					$sql=$con->prepare('SELECT * FROM protocolos WHERE id=:P1');
					$resultado=$sql->execute(array('P1'=>$_POST['id']));
					$resultado=$sql->fetchAll();
					$num=$sql->rowCount();
							...
							echo '
								...
								<div class="modal-body">
									<form class="forms-sample">
										...
										<button type="button" class="btn btn-primary mr-2" id="editarProtocolo" data-dismiss="modal">Guardar</button>
					...
					break;
				case 'editar':
					$sql=$con->prepare('UPDATE protocolos SET nombre=:P2, descripcion=:P3 WHERE id=:P1');
